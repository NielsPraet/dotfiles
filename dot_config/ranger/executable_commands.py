...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        if fd is not None:
            ...
            fzf_default_command = '{} . $HOME --follow {} {} {} --color=always'.format(
        ...
        env = os.environ.copy()
        env['FZF_DEFAULT_COMMAND'] = 'fd . $HOME --follow --exclude .git'
        env['FZF_DEFAULT_OPTS'] = '--color=16 --layout=reverse --border --padding 1 --preview="bat --style=numbers --color=always --line-range :500 {}" --bind shift-up:preview-page-up,shift-down:preview-page-down 2>/dev/null | head -n 100'
        # .format('''
            # (
                # batcat --color=always {} ||
                # bat --color=always {} ||
...
class fd_search(Command):
    ...
    def execute(self):
        ...
        hidden = ('--hidden' if self.fm.settings.show_hidden else '')
        exclude = "--no-ignore-vcs --exclude '.git' --exclude '*.py[co]' --exclude '__pycache__'"
        command = '{} . $HOME --follow {} {} {} --print0 {}'.format(
